...
export const TheProblemSection: React.FC = () => {
  ...
  return (
    <ScrollScene id="problem-scene" className="py-20 bg-[var(--bg-secondary)]/50 border-t-2 border-[var(--border-strong)] overflow-hidden">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        
        {/* Section Header with Subtle Camera Scale */}
        ...
        {/* 4 Illustrated Cards with Layered Staggered Entrance */}
        <ScrollLayer depth="midground" speed={-0.04} maxOffset={30}>
          <ScrollStaggerContainer className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12" staggerMs={85}>
            {scenarios.map((s, i) => {
              const Icon = s.icon;
              return (
                <ScrollStaggerItem key={i} index={i}>
                  <div
                    className="p-6 sm:p-7 rounded-3xl border-2 border-[var(--border-strong)] shadow-[4px_4px_0_0_#192138] bg-white flex flex-col justify-between h-full hover:translate-x-[-1px] hover:translate-y-[-1px] transition-all"
                  >
                    <div>
        ...
        {/* INCLUSA Removes the Barrier Banner with Foreground Accent */}
        <ScrollLayer depth="foreground" speed={0.07} maxOffset={35}>
          <ScrollReveal direction="up" distance={25} delay={80} duration={750}>
