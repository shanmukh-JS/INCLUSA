...
-- 1. Profiles: Strict user isolation
DROP POLICY IF EXISTS "Users can only access own profiles" ON accessibility_profiles;
CREATE POLICY "Users can only access own profiles" ON accessibility_profiles
  FOR ALL USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);

-- 2. Documents: Strict user isolation
...
-- 3. Analyses: Strict document owner isolation
DROP POLICY IF EXISTS "Users can only access own analyses" ON analyses;
CREATE POLICY "Users can only access own analyses" ON analyses
  FOR ALL USING (
    EXISTS (SELECT 1 FROM documents WHERE documents.id = analyses.document_id AND documents.user_id = auth.uid())
  )
  WITH CHECK (
    EXISTS (SELECT 1 FROM documents WHERE documents.id = analyses.document_id AND documents.user_id = auth.uid())
  );

-- 4. Chat Messages: Strict user isolation
DROP POLICY IF EXISTS "Users can only access own chat messages" ON chat_messages;
CREATE POLICY "Users can only access own chat messages" ON chat_messages
  FOR ALL USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);

-- 5. Reports: Strict document owner isolation
DROP POLICY IF EXISTS "Users can only access own reports" ON reports;
CREATE POLICY "Users can only access own reports" ON reports
  FOR ALL USING (
    EXISTS (SELECT 1 FROM documents WHERE documents.id = reports.document_id AND documents.user_id = auth.uid())
  )
  WITH CHECK (
    EXISTS (SELECT 1 FROM documents WHERE documents.id = reports.document_id AND documents.user_id = auth.uid())
  );
